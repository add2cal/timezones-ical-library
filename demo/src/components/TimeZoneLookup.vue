...
<template>
  <div class="to-primary-dark/40 from-primary-light/50 bg-radial py-20 shadow-inner">
    <div class="mx-auto flex w-full flex-col items-center gap-6 md:w-fit md:flex-row md:gap-0">
      ...
      <div class="relative z-50 w-full max-w-[360px] grow px-3">
        <Autocomplete v-model="selectedTimezone" />
      </div>
      <div class="w-52 shrink"></div>
    </div>

    <div :class="['mx-auto mt-8 w-full max-w-md px-3', showOutput ? 'block' : 'hidden']">
      <div class="rounded border border-gray-400 bg-gray-100 p-4 shadow-lg dark:border-gray-500 dark:bg-gray-800">
        <pre class="overflow-x-auto"><code class="text-sm">{{ outputValue }}</code></pre>
      </div>

      <div class="relative mt-8 flex rounded border border-gray-400 bg-gray-100 shadow-lg dark:border-gray-500 dark:bg-gray-800">
        <span class="flex items-center justify-center border-r border-gray-400 bg-black/10 p-2 text-xs font-bold dark:border-gray-500 dark:bg-white/10">Current Offset:</span>
